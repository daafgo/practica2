...
df <-dataFrame[dataFrame$country %in% paises,]
...
df[df$country=="Spain" & df$year==1999,]
unique(df$year)
```
...
para identificar valores extremos podriamos hacerlo de manera visual mediante un grafico como el que se muestra a continuación o hacer uso de valores centinela, como en nuestro caso se trata de unos datos oficiales no deberiamos encontrarnos con ninguno de estos problemas.
